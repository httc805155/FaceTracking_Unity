...
namespace Mediapipe
{
    public abstract class AnnotationController : MonoBehaviour
    {
        ...
        protected Vector3 ScaleVector_3D(Transform transform)
        {
            return new Vector3(10 * transform.localScale.x, 10 * transform.localScale.z, 10 * transform.localScale.y);
        }
        ...
        protected Vector3 GetPosition_3D(Transform screenTransform, NormalizedLandmark point, bool isFlipped)
        {
            var relX = (isFlipped ? -1 : 1) * (point.X - 0.5f);
            var relY = 0.5f - point.Y;
            var relZ = point.Z - 0.5f;

            var pos = Vector3.Scale(new Vector3(relX, relY, relZ), ScaleVector_3D(screenTransform)) 
                + screenTransform.localPosition;
            //var pos = new Vector3(relX, relY, relZ) * 50;
            return pos;
        }
